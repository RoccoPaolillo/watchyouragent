...
dffin <- merge(dfpropcr, dfpropng, by = c("turn","color","condition","week"))

dffinlong <- dffin %>%  pivot_longer(cols = c(5,6), names_to = c("variable"), values_to = "score")  %>% replace(is.na(.), 0) 

dffinlong <- dffinlong %>%
  mutate(score = ifelse(variable == "propccr",
                                 score,
                                 -1*score))

dffinlong %>% filter(condition == "pre",color == "rosso",turn == "BR01") %>%
  ggplot(aes(x = week, y = score, fill = variable))+
