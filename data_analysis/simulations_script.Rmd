...
```{r samescales, echo=FALSE, include = FALSE}
...
# lost of energy due to consumption from farmers' cow
# 22050 = number patches * max energy at time 0 = 441 * 50
...
datawide_nrgtot %>% filter(turn %in% c("BR02","BR03","BR04","BR05")) %>%
  ...
  scale_x_continuous(breaks = seq(0,35,7)) + 
    ...
    facet_grid(turn ~ factor(condition,levels = c("First Simulation","Second Simulation")),
             scales = "free_y") + 
  labs(x = "Days", y = "Natural resources lost due to Farmer's activity") + 
  theme_bw()
ggsave(file= "energylost.png",width = 5, height =5)
...
df <- df %>% filter(turn != "BR01") %>%   filter(condition == "Second Simulation" | condition == "First Simulation" & time <= 28)
...
# total energy consumed that week
...
dft$propnrgacq <- dft$nrgacqweek / dft$nrgacqtotweek
dft$propnrenrg <- dft$newnrgweek / dft$newnrgtotweek

dft %>% filter(week.x > 1) %>%
  ggplot(aes(x = week.x, y = actualnewrng,color = color)) +
  geom_line() +
  geom_point() + 
...
facet_wrap(~ turn + factor(condition, levels = c("First Simulation","Second Simulation")), scales = "free", ncol = 2) +
 ...
 # labs(y = "Energy renewal / Energy consumption", x = "week") + 
  theme_bw() 
ggsave(file= "relativeinvestment.png",width = 5, height =6)

# proportion of two dimensions


dftlong <- dft %>% filter(week.x > 1) %>%
  pivot_longer(cols = c(25,26), names_to = c("proportion"), values_to = "score")

dftlong %>%
  ggplot(aes(x = week.x, y = score,linetype = proportion, color = color)) +
  geom_line() +
  geom_point() + 
    scale_color_manual("gruppo", values=c(azzurro="cyan", blu="blue", rosso = "red",giallo = "orange",rosa = "pink")) +
 ...
 facet_wrap(~ turn + factor(condition, levels = c("First Simulation","Second Simulation")), scales = "free", ncol = 2) +
  scale_x_continuous(breaks = seq(2, 6, 1)) +
  labs(y = "Proportion to other groups", x = "week") + 
    guides(linetype = guide_legend(position =  "bottom")) +
  theme_bw() 
ggsave(file= "proportion2.png",width = 5, height =6)


dftlong %>% 
  filter(turn == "BR03") %>%
  ggplot(aes(y=score, x=week.x, color = color, linetype = condition,shape= proportion)) + 
  geom_line(aes(linetype = condition, color = color)) + 
  geom_point(aes(color = color, shape = proportion), size = 2) +
     scale_color_manual("gruppo", values=c(azzurro="cyan", blu="blue", rosso = "red",giallo = "orange",rosa = "pink")) +
  scale_shape_manual("proportion", values = c("propnrgacq" = "triangle","propnrenrg" = "square"), name = "indicator",
                        labels = c("propnrgacq" = "Energy consumption","propnrenrg" = "Energy renewal")) +
  scale_linetype_manual("condition", values = c("First Simulation" = "solid","Second Simulation" = "dashed"), name = "indicator") +
   facet_wrap(~ color, scales = "free_y", ncol = 1) + 
    scale_x_continuous(breaks = seq(2, 6, 1)) +
  labs(x = "week", y = "Proportion compared to other groups") + 
  theme_bw() +
  ggtitle("Turn BR03") + 
...
ggsave(file= "propcoopBR03_2.png",width = 6, height = 9)
...
long_df %>% filter(turn %in% c("BR02","BR03","BR04","BR05")) %>% filter(output == "Natural Resources") %>% ggplot(aes(x = time, y = score)) +
  geom_line(aes(color = turn), size =  1.2) +
...
facet_wrap(~ factor(condition, levels = c("First Simulation","Second Simulation")), scales = "free_y") +
  labs(title = NULL, x = "Days", y = "Natural resources",
       subtitle = "Including energy renewal and constant growth") +
  theme_bw( ) +
  theme(
