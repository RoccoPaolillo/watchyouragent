...
dft$ownnrgweek <- dft$ccr.x * 10

dft$actualcontr <- dft$ownnrgweek - dft$nrgacqweek
dft$actualcontrprop <- dft$ownnrgweek / dft$nrgacqweek
dft$actualnrgacprop <- dft$nrgacqweek / dft$ownnrgweek

ggplot(dft,aes(x = week.x, y = actualnrgacprop, color = color)) +
  geom_line() +
  geom_point() + 
    scale_color_manual("gruppo", values=c(azzurro="cyan", blu="blue", rosso = "red",giallo = "yellow",rosa = "pink")) +
facet_wrap(~ turn + factor(condition, levels = c("pre","post")), scales = "free_y") +
  theme_bw()
